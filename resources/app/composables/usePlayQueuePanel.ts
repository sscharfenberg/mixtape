/******************************************************************************
 * usePlayQueuePanel
 * Whether the play queue's panel is showing on a narrow screen.
 *
 ...
 *
 * Only the narrow layout reads this. At `landscape` and up the panel is an
 * ordinary column in the grid, always shown while the queue has anything in it,
 * and there is no toggle at all — see PlayQueue's styles.
 *****************************************************************************/
...
export type UsePlayQueuePanelReturn = {
    /** Whether the panel is showing. Only consulted below the `landscape` step. */
...
export function usePlayQueuePanel(): UsePlayQueuePanelReturn {
    ...
    return { isOpen: computed(() => open.value), toggle, close };
}

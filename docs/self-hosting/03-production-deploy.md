...
The box is built at this point, and what you run from here on is a different subject: deploying a
change, rebuilding dev, running artisan against either site, copying the collection onto a disk.
Those four scripts are documented together in [`commands.md`](commands.md) — two of them run on the
server and two on your workstation, which is the distinction that matters most when something goes
wrong.

The rest of this page is the machinery that then runs without you: the rate limits in front of the
app, and the scheduled jobs.
...
> **Put the site in maintenance first** ([`mt artisan down --prod`](commands.md#driving-both-sites--mt)). Not enforced — a
...
- **No queue worker.** Nothing in the app implements `ShouldQueue` and mail is sent synchronously, so
  there is no queue for a worker to drain. A systemd unit for one belongs to whichever change
  introduces the first queued job.

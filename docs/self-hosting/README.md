...
| A sync to a USB stick appears to hang after the last file | Not the copy — an `xattr -rc` pass over the tree, which takes minutes on twelve thousand entries and removes nothing that deleting the `._` sidecars has not already removed. On FAT the sidecar **is** where the attribute is stored | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| `--mirror` wants to delete every file with an accent in its name | macOS writes filenames to FAT **decomposed** while the server stores them precomposed. Lookups are normalisation-insensitive so copies are fine; a directory listing is not, so rsync calls a correctly-copied file extraneous. Probe the volume and refuse to mirror | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| A `set -e` script dies silently right after reading a state file | `read` returns non-zero when it hits EOF without its delimiter — having already assigned every field. A file with no trailing newline therefore kills the script with no message. `read … || true` | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| `awk: towc: multibyte conversion failure` while filtering rsync's output | rsync escapes *some* non-ASCII bytes in a filename as `\#NNN` and passes others raw, so its itemized output is not valid UTF-8. Harmless but deafening — run the filter under `LC_ALL=C` | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| A handful of albums arrive on the stick with no cover art | rsync's filter patterns are **case-sensitive** while the app's extension match is not, so `--include='*.jpg'` silently skips a `.JPG`. Write the globs as character classes | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| Every rsync to a USB stick re-copies the whole library | FAT records mtimes to a two-second granularity, so without `--modify-window=1` every file looks changed on every run — and nothing reports a problem | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| A car head unit lists thousands of 4 KB phantom tracks | macOS spills the extended attribute it stamps on files it creates into a `._` AppleDouble sidecar, because FAT cannot store one. An `--exclude` cannot stop it — they are created on the destination, not sent from the source — and `dot_clean` does not finish the job | [03](03-production-deploy.md#the-decisions-worth-knowing-1) |
| `rsync: unknown option --no-perms` (or `--info=progress2`, `--dry-run`) on macOS | macOS ships **openrsync**, which reports itself as "rsync version 2.6.9 compatible" and is not rsync 3.x. Build up from `-rt` rather than subtracting from `-a`; `-n` exists where `--dry-run` does not | [03](03-production-deploy.md#macos-ships-openrsync-and-it-is-missing-flags-you-will-reach-for) |
| A remote path containing a space arrives as two paths that do not exist | rsync hands the remote path to a shell on the far side, and openrsync has no `--protect-args`. Escape the space inside the quotes | [03](03-production-deploy.md#macos-ships-openrsync-and-it-is-missing-flags-you-will-reach-for) |

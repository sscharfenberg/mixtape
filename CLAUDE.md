...
**`.m3u8` for UTF-8 exports.** Not settled: whether Android's stricter readers want the file named
`.m3u8` when its bytes are UTF-8 (the convention) rather than the `.m3u` this always writes. It is a
one-line change in `PlaylistExport::filename()` if a device actually needs it — don't build it on the
convention alone. Export presets now name the device, so the preset is where such a decision would
hang if a real reader turns out to need it.
